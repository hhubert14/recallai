...
export class DrizzleQuestionRepository implements IQuestionRepository {
  ...
  async findQuestionById(
    questionId: number
  ): Promise<MultipleChoiceQuestionEntity | null> {
  ...
  async countQuestionsByVideoIds(
    videoIds: number[]
  ...
  async deleteQuestion(questionId: number): Promise<void> {
    await dbRetry(() =>
      this.db.delete(questions).where(eq(questions.id, questionId))
    );
  }
