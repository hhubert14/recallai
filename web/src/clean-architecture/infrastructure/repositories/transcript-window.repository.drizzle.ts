  ...
  implements ITranscriptWindowRepository
{
  async createWindowsBatch(
    ...
    const data = await dbRetry(() =>
      db.insert(transcriptWindows).values(windows).returning()
    );

    return data.map((window) => this.toEntity(window));
  }

  async findWindowsByVideoId(
    videoId: number
  ): Promise<TranscriptWindowEntity[]> {
    const data = await dbRetry(() =>
      db
  ...
  async findMostSimilarWindow(
    ...
    const results = await this.findTopKSimilarWindows(
      videoId,
      queryEmbedding,
      1
    );
  ...
  async deleteWindowsByVideoId(videoId: number): Promise<void> {
    await dbRetry(() =>
      db.delete(transcriptWindows).where(eq(transcriptWindows.videoId, videoId))
    );
  }
