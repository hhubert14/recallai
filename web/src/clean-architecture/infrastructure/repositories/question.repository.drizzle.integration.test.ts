...
describe("DrizzleQuestionRepository (integration)", () => {
  ...
  describe("updateQuestion", () => {
    ...
    it("persists changes after update", async () => {
      ...
      expect(fetched!.options.find((opt) => opt.isCorrect)?.optionText).toBe(
        "New 3"
      );
    });
    ...
    it("updates all options atomically", async () => {
      ...
      const updatedOptions = [
        {
          id: created.options[0].id,
          optionText: "New A",
          isCorrect: true,
          explanation: "Now correct",
        },
        {
          id: created.options[1].id,
          optionText: "New B",
          isCorrect: false,
          explanation: null,
        },
        {
          id: created.options[2].id,
          optionText: "New C",
          isCorrect: false,
          explanation: null,
        },
        {
          id: created.options[3].id,
          optionText: "New D",
          isCorrect: false,
          explanation: null,
        },
      ...
      expect(
        result.options.filter((opt) => opt.optionText.startsWith("New"))
      ).toHaveLength(4);
      expect(result.options.filter((opt) => opt.isCorrect)).toHaveLength(1);
      expect(result.options.find((opt) => opt.isCorrect)?.optionText).toBe(
        "New A"
      );
    });
  });
