...
export class YoutubeTranscriptVideoTranscriptService
  implements IVideoTranscriptService
{
  ...
  private handleError(error: unknown, videoId: string): void {
    if (error instanceof Error) {
      if (error.message.includes("unavailable")) {
        logger.video.warn("Video unavailable for transcript extraction", {
          videoId,
          error: error.message,
        });
        ...
        logger.video.error("Unexpected error fetching transcript", error, {
          videoId,
        });
      }
    } else {
      logger.video.error("Unknown error type fetching transcript", error, {
        videoId,
      });
    }
  }
