...
describeIntegration(
  "YoutubeTranscriptVideoTranscriptService (integration)",
  () => {
    beforeAll(() => {
      if (!API_KEY) {
        throw new Error(
          "YOUTUBE_TRANSCRIPT_API_KEY is required for integration tests"
        );
      }
    });
    ...
    it("shows parsed output", async () => {
      ...
      parsed.segments.slice(0, 3).forEach((seg, i) => {
        console.log(
          `   ${i + 1}. [${seg.startTime}s - ${seg.endTime}s] "${seg.text}"`
        );
      });
