...
const flashcardSchema = z.object({
  ...
  front: z.string().describe("A clear question or prompt about a key concept"),
  back: z.string().describe("A concise, accurate answer (1-3 sentences)"),
});
  ...
  implements ISuggestionGeneratorService
{
  async generate(
    ...
    if (!prompt) {
      logger.video.warn("Missing required prompt for suggestion generation");
      return undefined;
    }
...
- Ensure all content is factually accurate`,
      ...
      logger.video.error("Error generating suggestions", error, {
        prompt,
