...
function createMockQuestion(
  ...
  return new MultipleChoiceQuestionEntity(id, videoId, `Question ${id}`, [
...
describe("GetQuizCompletionStatusUseCase", () => {
  ...
  beforeEach(() => {
    ...
    useCase = new GetQuizCompletionStatusUseCase(
      mockQuestionRepo,
      mockAnswerRepo
    );
  });
  ...
  describe("happy path", () => {
    it("returns true when user answered all questions for a video", async () => {
      ...
      vi.mocked(
        mockAnswerRepo.findAnsweredQuestionIdsByVideoId
    ...
    it("returns false when user answered some but not all questions", async () => {
      ...
      vi.mocked(
        mockAnswerRepo.findAnsweredQuestionIdsByVideoId
      ).mockResolvedValue([1, 2]); // Missing question 3
