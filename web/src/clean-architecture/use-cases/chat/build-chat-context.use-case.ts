...
export class BuildChatContextUseCase {
  ...
  async execute(
    ...
    if (!summary) {
      throw new Error(
        "Video summary not found. Please process the video first."
      );
    }
    ...
    try {
      ...
      const windowMatches =
        await this.transcriptWindowRepository.findTopKSimilarWindows(
          videoId,
          queryEmbedding,
          TOP_K_WINDOWS
        );
      ...
      logger.warn(
        "[CHAT] Embedding retrieval failed, continuing with empty context",
        {
          videoId,
          error: err,
        }
