...
describe("EditQuestionUseCase", () => {
  ...
  it("updates a question successfully", async () => {
    ...
    vi.mocked(mockQuestionRepository.findQuestionById).mockResolvedValue(
      existingQuestion
    );
    vi.mocked(
      mockReviewableItemRepository.findReviewableItemByQuestionId
    ).mockResolvedValue(reviewableItem);
    vi.mocked(mockQuestionRepository.updateQuestion).mockResolvedValue(
      updatedQuestion
    );
    ...
    expect(mockQuestionRepository.findQuestionById).toHaveBeenCalledWith(
      questionId
    );
    expect(
      mockReviewableItemRepository.findReviewableItemByQuestionId
  ...
  it("throws 'Not authorized to edit this question' when user does not own the question", async () => {
    ...
    vi.mocked(
      mockReviewableItemRepository.findReviewableItemByQuestionId
    ).mockResolvedValue(otherUserReviewableItem);
  ...
  it("trims whitespace from questionText and optionText before saving", async () => {
    ...
    await useCase.execute({
      ...
      options: [
        {
          id: 1,
          optionText: "  Trimmed A  ",
          isCorrect: true,
          explanation: null,
        },
        {
          id: 2,
          optionText: "  Trimmed B  ",
          isCorrect: false,
          explanation: null,
        },
        {
          id: 3,
          optionText: "  Trimmed C  ",
          isCorrect: false,
          explanation: null,
        },
        {
          id: 4,
          optionText: "  Trimmed D  ",
          isCorrect: false,
          explanation: null,
        },
      ],
  ...
  it("updates a question with videoId", async () => {
    ...
    vi.mocked(
      mockReviewableItemRepository.findReviewableItemByQuestionId
    ).mockResolvedValue(reviewableItemWithVideo);
    vi.mocked(mockQuestionRepository.updateQuestion).mockResolvedValue(
      updatedQuestion
    );
