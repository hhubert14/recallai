...
describe("AddQuestionToStudySetUseCase", () => {
  ...
  it("adds a question to a manual study set (no videoId)", async () => {
    ...
    vi.mocked(
      mockReviewableItemRepository.countItemsByStudySetId
    ).mockResolvedValue(0);
    vi.mocked(
      mockQuestionRepository.createMultipleChoiceQuestion
    ).mockResolvedValue(createdQuestion);
    vi.mocked(
      mockReviewableItemRepository.createReviewableItemsForQuestionsBatch
    ...
    expect(
      mockQuestionRepository.createMultipleChoiceQuestion
    ).toHaveBeenCalledWith(null, "What is TDD?", validOptions);
    expect(
      mockReviewableItemRepository.createReviewableItemsForQuestionsBatch
  ...
  it("adds a question to a video-sourced study set (with videoId)", async () => {
    ...
    expect(
      mockQuestionRepository.createMultipleChoiceQuestion
    ).toHaveBeenCalledWith(42, "What is React?", validOptions);
    expect(
      mockReviewableItemRepository.createReviewableItemsForQuestionsBatch
    ).toHaveBeenCalledWith([
      {
  ...
  it("throws error when study set has reached item limit", async () => {
    ...
    vi.mocked(
      mockReviewableItemRepository.countItemsByStudySetId
    ).mockResolvedValue(STUDY_SET_ITEM_LIMIT);
  ...
  it("allows adding question when under item limit", async () => {
    ...
    vi.mocked(
      mockReviewableItemRepository.countItemsByStudySetId
    ).mockResolvedValue(STUDY_SET_ITEM_LIMIT - 1);
    vi.mocked(
      mockQuestionRepository.createMultipleChoiceQuestion
    ).mockResolvedValue(createdQuestion);
    vi.mocked(
