...
describe("GetStudySetItemsUseCase", () => {
    ...
    it("returns flashcards for a study set with only flashcards", async () => {
        const reviewableItems = [
            new ReviewableItemEntity(1, userId, "flashcard", null, 100, null, studySetId, "2025-01-27"),
            new ReviewableItemEntity(2, userId, "flashcard", null, 101, null, studySetId, "2025-01-27"),
        ];

        const flashcard1 = new FlashcardEntity(100, null, userId, "Front 1", "Back 1", "2025-01-27");
        ...
        vi.mocked(mockFlashcardRepo.findFlashcardsByIds).mockResolvedValue([flashcard1, flashcard2]);

        const result = await useCase.execute(studySetId);

        expect(result.items).toHaveLength(2);
        expect(result.items[0]).toEqual({ itemType: "flashcard", flashcard: flashcard1 });
        expect(result.items[1]).toEqual({ itemType: "flashcard", flashcard: flashcard2 });
        expect(mockFlashcardRepo.findFlashcardsByIds).toHaveBeenCalledWith([100, 101]);
        expect(mockQuestionRepo.findQuestionsByIds).not.toHaveBeenCalled();
    });

    it("returns questions for a study set with only questions", async () => {
        const reviewableItems = [
            new ReviewableItemEntity(1, userId, "question", 200, null, null, studySetId, "2025-01-27"),
            new ReviewableItemEntity(2, userId, "question", 201, null, null, studySetId, "2025-01-27"),
        ];
        ...
        expect(result.items).toHaveLength(2);
        expect(result.items[0]).toEqual({ itemType: "question", question: question1 });
        expect(result.items[1]).toEqual({ itemType: "question", question: question2 });
        expect(mockFlashcardRepo.findFlashcardsByIds).not.toHaveBeenCalled();
        expect(mockQuestionRepo.findQuestionsByIds).toHaveBeenCalledWith([200, 201]);
    });
    ...
    it("returns items in reviewable_items order (preserves insertion order)", async () => {
        ...
        const reviewableItems = [
            new ReviewableItemEntity(1, userId, "flashcard", null, 100, null, studySetId, "2025-01-27"),
            new ReviewableItemEntity(2, userId, "question", 200, null, null, studySetId, "2025-01-27"),
            new ReviewableItemEntity(3, userId, "flashcard", null, 101, null, studySetId, "2025-01-27"),
        ];
        ...
        vi.mocked(mockFlashcardRepo.findFlashcardsByIds).mockResolvedValue([flashcard2, flashcard1]);
        vi.mocked(mockQuestionRepo.findQuestionsByIds).mockResolvedValue([question1]);
        ...
        expect(result.items[0]).toEqual({ itemType: "flashcard", flashcard: flashcard1 });
        expect(result.items[1]).toEqual({ itemType: "question", question: question1 });
        expect(result.items[2]).toEqual({ itemType: "flashcard", flashcard: flashcard2 });
    });

    it("works with video-sourced study sets (items have videoId)", async () => {
        const videoId = 42;
        const reviewableItems = [
            new ReviewableItemEntity(1, userId, "flashcard", null, 100, videoId, studySetId, "2025-01-27"),
        ];

        const flashcard = new FlashcardEntity(100, videoId, userId, "Front", "Back", "2025-01-27");

        vi.mocked(mockReviewableItemRepo.findReviewableItemsByStudySetId).mockResolvedValue(reviewableItems);
        vi.mocked(mockFlashcardRepo.findFlashcardsByIds).mockResolvedValue([flashcard]);

        const result = await useCase.execute(studySetId);

        expect(result.items).toHaveLength(1);
        expect(result.items[0]).toEqual({ itemType: "flashcard", flashcard });
        expect((result.items[0] as { itemType: "flashcard"; flashcard: FlashcardEntity }).flashcard.videoId).toBe(videoId);
    });
});
