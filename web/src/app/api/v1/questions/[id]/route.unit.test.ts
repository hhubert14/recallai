...
describe("PATCH /api/v1/questions/[id]", () => {
  ...
  it("returns 403 when user not authorized", async () => {
    ...
    const mockExecute = vi
      .fn()
      .mockRejectedValue(new Error("Not authorized to edit this question"));
    vi.mocked(EditQuestionUseCase).mockImplementation(
      () =>
        ({
          execute: mockExecute,
  ...
  it("returns 400 when question text is empty", async () => {
    ...
    const mockExecute = vi
      .fn()
      .mockRejectedValue(new Error("Question text cannot be empty"));
    vi.mocked(EditQuestionUseCase).mockImplementation(
      () =>
        ({
          execute: mockExecute,
  ...
  it("returns 400 when question text exceeds 1000 characters", async () => {
    ...
    const mockExecute = vi
      .fn()
      .mockRejectedValue(
        new Error("Question text cannot exceed 1000 characters")
      );
    vi.mocked(EditQuestionUseCase).mockImplementation(
      () =>
        ({
  ...
  it("returns 400 when options count is wrong", async () => {
    ...
    const mockExecute = vi
      .fn()
      .mockRejectedValue(new Error("Must provide exactly 4 options"));
    vi.mocked(EditQuestionUseCase).mockImplementation(
      () =>
        ({
          execute: mockExecute,
  ...
  it("returns 400 when option text is empty", async () => {
    ...
    const mockExecute = vi
      .fn()
      .mockRejectedValue(new Error("Option text cannot be empty"));
    vi.mocked(EditQuestionUseCase).mockImplementation(
      () =>
        ({
          execute: mockExecute,
  ...
  it("returns 400 when option text exceeds 500 characters", async () => {
    ...
    const mockExecute = vi
      .fn()
      .mockRejectedValue(new Error("Option text cannot exceed 500 characters"));
    vi.mocked(EditQuestionUseCase).mockImplementation(
      () =>
        ({
          execute: mockExecute,
  ...
  it("returns 400 when option ID does not belong to the question", async () => {
    ...
    const mockExecute = vi
      .fn()
      .mockRejectedValue(
        new Error("Invalid option ID: option does not belong to this question")
      );
    vi.mocked(EditQuestionUseCase).mockImplementation(
      () =>
        ({
    ...
    expect(data.status).toBe("fail");
    expect(data.data.error).toBe(
      "Invalid option ID: option does not belong to this question"
    );
  });
