...
vi.mock("@/clean-architecture/use-cases/question/add-question-to-study-set.use-case");
...
describe("POST /api/v1/study-sets/[publicId]/questions", () => {
    ...
    it("returns 401 if not authenticated", async () => {
        ...
        const request = createMockRequest({ questionText: "What is TDD?", options: validOptions });
    ...
    it("returns 400 if options is not an array", async () => {
        ...
        const request = createMockRequest({ questionText: "What is TDD?", options: "not-an-array" });
    ...
    it("creates question and returns 201", async () => {
        ...
        expect(data.data.question).toEqual({
            ...
            options: [
                { id: 1, optionText: "Option A", isCorrect: true, explanation: "Correct!" },
    ...
    it("returns 400 when validation fails", async () => {
        ...
        const mockExecute = vi.fn().mockRejectedValue(new Error("Question must have exactly 4 options"));
        vi.mocked(AddQuestionToStudySetUseCase).mockImplementation(() => ({
            execute: mockExecute,
        }) as unknown as AddQuestionToStudySetUseCase);
