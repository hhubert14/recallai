...
import {
  BuildPracticeContextUseCase,
  ConceptInput,
} from "@/clean-architecture/use-cases/practice/build-practice-context.use-case";
...
export async function POST(
  ...
  try {
    ...
    // Rate limiting
    const { success } = await getRateLimiter("/api/v1/practice/chat").limit(
      user.id
    );
    if (!success) {
      return jsendFail(
    ...
    const {
      messages,
      concept,
    }: { messages: UIMessage[]; concept: ConceptInput } = body;
    ...
    if (!concept.description || typeof concept.description !== "string") {
      return jsendFail(
        { error: "Missing or invalid concept description" },
        400
      );
    }
