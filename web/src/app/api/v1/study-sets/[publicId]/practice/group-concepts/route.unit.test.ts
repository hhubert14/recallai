...
vi.mock(
  "@/clean-architecture/use-cases/practice/group-items-into-concepts.use-case",
  () => ({
...
vi.mock(
  "@/clean-architecture/infrastructure/services/concept-grouper.service.langchain",
  () => ({
...
function createMockRequest(): NextRequest {
  return new NextRequest(
    "http://localhost:3000/api/v1/study-sets/abc123/practice/group-concepts",
    {
      method: "POST",
    }
  );
...
describe("POST /api/v1/study-sets/[publicId]/practice/group-concepts", () => {
  ...
  it("returns concepts on success", async () => {
    ...
    const response = await POST(createMockRequest(), {
      params: createParams("test-public-id"),
    });
    const data = await response.json();
