...
describe("TermsList", () => {
  ...
  it("renders header with correct term count", () => {
    render(
      <TermsList
        ...
        studySetPublicId={mockStudySetPublicId}
      />
    );
    ...
    const questionsOnly: TermWithMastery[] = [
      {
        ...
        question: {
          ...
          options: [
            { id: 1, optionText: "Answer", isCorrect: true, explanation: null },
          ],
        },
        masteryStatus: "not_started",
      },
  ...
  it("renders Review button with link to review page", () => {
    ...
    expect(reviewLink).toHaveAttribute(
      "href",
      `/dashboard/study-set/${mockStudySetPublicId}/review`
    );
  });
  ...
  it("shows due count in Review button when dueCount > 0", () => {
    ...
    expect(
      screen.getByRole("link", { name: /review \(5\)/i })
    ).toBeInTheDocument();
  });
