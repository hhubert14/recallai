...
describe("TermCard", () => {
  ...
  describe("edit functionality", () => {
    ...
    it("renders edit button for flashcard terms when onEditFlashcard is provided", () => {
      ...
      render(
        <TermCard term={flashcardTerm} onEditFlashcard={onEditFlashcard} />
      );

      expect(
        screen.getByRole("button", { name: /edit flashcard/i })
      ).toBeInTheDocument();
    });
    ...
    it("calls onEditQuestion with question data when edit button is clicked", async () => {
      ...
      render(
        <TermCard
          term={questionTermWithAllOptions}
  ...
  describe("inline edit mode for flashcard", () => {
    ...
    it("calls onEditedContentChange when front text is modified", async () => {
      ...
      const lastCall =
        onEditedContentChange.mock.calls[
          onEditedContentChange.mock.calls.length - 1
        ][0];
    ...
    it("displays character count for question text field in edit mode", () => {
      const content: EditedTermContent = {
        ...
        options: [
          {
            id: 1,
            optionText: "Test-Driven Development",
            isCorrect: true,
            explanation: null,
          },
          {
    ...
    it("displays character count for each option field in edit mode", () => {
      const content: EditedTermContent = {
        ...
        options: [
          ...
          {
            id: 2,
            optionText: "Medium length",
            isCorrect: false,
            explanation: null,
          }, // 13 characters
          {
            id: 3,
            optionText: "A bit longer option",
            isCorrect: false,
            explanation: null,
          }, // 19 characters
          {
            id: 4,
            optionText: "The longest option text here",
            isCorrect: false,
            explanation: null,
          }, // 28 characters
        ],
  ...
  describe("delete functionality", () => {
    ...
    it("renders delete button for flashcard when onDeleteFlashcard is provided", () => {
      ...
      render(
        <TermCard term={flashcardTerm} onDeleteFlashcard={onDeleteFlashcard} />
      );

      expect(
        screen.getByRole("button", { name: /delete flashcard/i })
      ).toBeInTheDocument();
    });
    ...
    it("shows confirmation dialog when delete button is clicked for flashcard", async () => {
      ...
      expect(
        screen.getByText(/are you sure you want to delete this flashcard/i)
      ).toBeInTheDocument();
    });
    ...
    it("renders delete button for question when onDeleteQuestion is provided", () => {
      ...
      render(
        <TermCard term={questionTerm} onDeleteQuestion={onDeleteQuestion} />
      );
    ...
    it("does not render delete button when onDeleteQuestion is not provided", () => {
      ...
      expect(
        screen.queryByRole("button", { name: /delete question/i })
      ).not.toBeInTheDocument();
    });
    ...
    it("shows confirmation dialog when delete button is clicked for question", async () => {
      ...
      expect(screen.getByRole("alertdialog")).toBeInTheDocument();
      expect(
        screen.getByText(/are you sure you want to delete this question/i)
      ).toBeInTheDocument();
    });
