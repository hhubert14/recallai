  ...
  if (currentItem.itemType === "flashcard" && currentItem.flashcard) {
    ...
    return (
      <div className="space-y-6">
        ...
        <QuizProgress current={currentIndex + 1} total={items.length} />
        <SingleFlashcardItem
          key={flashcard.id}
          flashcard={flashcard}
...
function SingleFlashcardItem({
  flashcard,
  onComplete,
}: SingleFlashcardItemProps) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3">
        {!showResult ? (
          ...
          <Button
            onClick={() => onComplete(selectedOption?.isCorrect ?? false)}
            size="lg"
          >
            Next →
          </Button>
