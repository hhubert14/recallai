...
describe("PracticeModal", () => {
  ...
  it("displays concepts for selection after loading", async () => {
    ...
    expect(
      screen.getByText("How plants convert light into energy")
    ).toBeInTheDocument();
  ...
  it("allows user to select a concept", async () => {
    ...
    const photosynthesisOption = screen.getByRole("radio", {
  ...
  it("transitions to chat phase after selecting concept and clicking start", async () => {
    ...
    expect(
      screen.getByPlaceholderText(/explain the concept/i)
    ).toBeInTheDocument();
  });
  ...
  it("shows error message when concept loading fails", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/failed to analyze study set/i)
      ).toBeInTheDocument();
    });
  ...
  it("shows feedback after ending session", async () => {
    ...
    vi.mocked(global.fetch).mockImplementation((url) => {
      ...
      if (
        typeof url === "string" &&
        url.includes("/practice/generate-feedback")
      ) {
        return Promise.resolve({
          ok: true,
          json: async () => ({
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Great job explaining the concept!")
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows Practice Another and Done buttons in feedback phase", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByRole("button", { name: /practice another/i })
      ).toBeInTheDocument();
    });
