...
const createMockFlashcardSuggestion = (
  id: string,
  front: string,
  back: string
): Suggestion => ({
...
const createMockQuestionSuggestion = (
  id: string,
  questionText: string
): Suggestion => ({
  tempId: id,
...
describe("AIGenerateModal", () => {
  ...
  describe("prompt suggestion chips", () => {
    ...
    it("fills prompt when suggestion chip is clicked", async () => {
      ...
      expect((textarea as HTMLTextAreaElement).value.toLowerCase()).toContain(
        "key terms"
      );
    });
  });
  ...
  describe("item type selector", () => {
    ...
    it("defaults to mix mode", () => {
      ...
      const mixButton = screen.getByRole("radio", {
        name: /mix \(ai decides\)/i,
      });
    ...
    it("allows selecting a different item type", async () => {
      ...
      const questionsButton = screen.getByRole("radio", {
        name: /questions only/i,
      });
  ...
  describe("form submission", () => {
    ...
    it("shows error when API call fails", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/failed to generate suggestions/i)
        ).toBeInTheDocument();
      });
    });
    ...
    it("shows loading state while generating", async () => {
      ...
      resolvePromise!({
        ok: true,
        json: () =>
          Promise.resolve({ status: "success", data: { suggestions: [] } }),
      });
    });
  ...
  describe("review phase", () => {
    ...
    it("shows accept, reject, and edit buttons for each suggestion", async () => {
      ...
      expect(
        within(suggestionCard).getByRole("button", { name: /^accept$/i })
      ).toBeInTheDocument();
      expect(
        within(suggestionCard).getByRole("button", { name: /^reject$/i })
      ).toBeInTheDocument();
      expect(
        within(suggestionCard).getByRole("button", { name: /^edit$/i })
      ).toBeInTheDocument();
    });
    ...
    it("removes suggestion from list when rejected", async () => {
      ...
      const rejectButton = within(firstCard as HTMLElement).getByRole(
        "button",
        { name: /reject/i }
      );
    ...
    it("shows error when study set has reached item limit on Accept All", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/has reached the maximum limit/i)
        ).toBeInTheDocument();
      });
    ...
    it("updates progress indicator when suggestions are accepted", async () => {
      ...
      const firstCard = screen
        .getByText("Card 1")
        .closest('[data-testid="suggestion-card"]');
      const acceptButton = within(firstCard as HTMLElement).getByRole(
        "button",
        { name: /accept/i }
      );
    ...
    it("displays error message when accepting suggestion fails", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole("alert")).toHaveTextContent(
          "Front text exceeds maximum length"
        );
      });
