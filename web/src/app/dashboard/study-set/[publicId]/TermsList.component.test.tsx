...
describe("TermsList", () => {
    it("renders header with correct term count", () => {
        render(<TermsList terms={mockTerms} onStudy={vi.fn()} />);
    ...
    it("calls onStudy when a study mode is selected", async () => {
        ...
        render(<TermsList terms={mockTerms} onStudy={handleStudy} />);
    ...
    it("disables flashcards mode when no flashcards exist", () => {
        ...
        render(<TermsList terms={questionsOnly} onStudy={vi.fn()} />);
    ...
    it("disables quiz mode when no questions exist", () => {
        ...
        render(<TermsList terms={flashcardsOnly} onStudy={vi.fn()} />);
        expect(screen.getByText("(1)")).toBeInTheDocument();
    });

    it("renders empty state when no terms exist", () => {
        render(<TermsList terms={[]} onStudy={vi.fn()} />);
