...
export const studySets = pgTable(
  ...
  (table) => [
    ...
    check(
      "study_sets_source_type_check",
      sql`source_type IN ('video', 'manual', 'pdf')`
    ),
    index("idx_study_sets_user_id").using(
      "btree",
      table.userId.asc().nullsLast()
    ),
    index("idx_study_sets_video_id").using(
      "btree",
...
export const transcriptWindows = pgTable(
  "transcript_windows",
  {
    id: bigint({ mode: "number" })
      .primaryKey()
      .generatedByDefaultAsIdentity({
        name: "transcript_windows_id_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
        cache: 1,
  ...
  (table) => [
    foreignKey({
      ...
      .onUpdate("cascade")
      .onDelete("cascade"),
    index("idx_transcript_windows_video_id").using(
      "btree",
      table.videoId.asc().nullsLast()
    ),
    unique("transcript_windows_video_id_window_index_key").on(
      table.videoId,
      table.windowIndex
    ),
  ]
...
export const videoTranscripts = pgTable(
  ...
  (table) => [
    foreignKey({
      ...
      .onUpdate("cascade")
      .onDelete("cascade"),
    index("idx_video_transcripts_video_id").using(
      "btree",
      table.videoId.asc().nullsLast()
    ),
...
export const chatMessages = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onUpdate("cascade")
      .onDelete("cascade"),
    index("idx_chat_messages_video_user").using(
      "btree",
      table.videoId.asc().nullsLast(),
      table.userId.asc().nullsLast()
    ),
...
export const reviewableItems = pgTable(
  "reviewable_items",
  {
    id: bigint({ mode: "number" })
      .primaryKey()
      .generatedByDefaultAsIdentity({
        name: "reviewable_items_id_seq",
        startWith: 1,
        increment: 1,
        minValue: 1,
        cache: 1,
  ...
  (table) => [
    ...
    index("idx_reviewable_items_user_id").using(
      "btree",
      table.userId.asc().nullsLast()
    ),
    index("idx_reviewable_items_video_id").using(
      "btree",
      table.videoId.asc().nullsLast()
    ),
    index("idx_reviewable_items_study_set_id").using(
      "btree",
      table.studySetId.asc().nullsLast()
    ),
  ]
);
...
export const reviewProgress = pgTable(
  "review_progress",
  {
    ...
    userId: uuid("user_id").notNull(),
    reviewableItemId: bigint("reviewable_item_id", {
      mode: "number",
    }).notNull(),
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onUpdate("cascade")
      .onDelete("cascade"),
    unique("review_progress_user_reviewable_item_key").on(
      table.userId,
      table.reviewableItemId
    ),
    index("idx_review_progress_user_id").using(
      "btree",
      table.userId.asc().nullsLast()
    ),
    index("idx_review_progress_next_review_date").using(
      "btree",
      table.nextReviewDate.asc().nullsLast()
    ),
  ]
);
...
export const folderStudySets = pgTable(
  ...
  (table) => [
    ...
    foreignKey({
      ...
      .onUpdate("cascade")
      .onDelete("cascade"),
    unique("folder_study_sets_folder_study_set_key").on(
      table.folderId,
      table.studySetId
    ),
    index("idx_folder_study_sets_folder_id").using(
      "btree",
      table.folderId.asc().nullsLast()
    ),
    index("idx_folder_study_sets_study_set_id").using(
      "btree",
      table.studySetId.asc().nullsLast()
    ),
  ]
