...
} from "@/data-access/users/stripe-customer";
...
export const subscribeAction = async ({ userId }: Props) => {
    try {
        ...
        if (userError || !userData) {
            logger.subscription.error("Error getting user data", userError, {
                userId,
            });
            return null;
        }
        ...
        logger.subscription.error("Error during subscription action", error, {
            userId,
        });
        return null;
    }
