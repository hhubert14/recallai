...
import { questions, videos, questionOptions, userAnswers, userQuestionProgress } from "@/drizzle/schema";
...
export async function getRandomAnsweredQuestions(
    ...
    try {
        // Get questions that the user has answered
        const rows = await db
            .select()
            .from(questions)
        ...
        // Group by question and collect options (removing duplicates)
        type ProgressData = {
            id: number;
            userId: string;
            questionId: number;
        ...
        for (const row of rows) {
            ...
            if (row.question_options) {
                ...
                const optionExists = questionsMap[questionId].options.find(
                    (opt) => opt.id === row.question_options!.id
                );
                if (!optionExists) {
        ...
        // Get unique questions, shuffle and take limit
