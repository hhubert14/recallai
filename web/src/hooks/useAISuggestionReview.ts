...
export function useAISuggestionReview({
        ...
        try {
            if (suggestion.itemType === "flashcard") {
                ...
                if (response.ok && data.status === "success") {
                    ...
                    setError(data.data?.error || "Failed to add flashcard. Please try again.");
                }
            } else {
                const response = await fetch(
                ...
                if (response.ok && data.status === "success") {
                    ...
                    setError(data.data?.error || "Failed to add question. Please try again.");
                }
            }
        } catch {
    ...
    const acceptAll = async () => {
        ...
        try {
            ...
            if (remaining <= 0) {
                setError(`Study set has reached the maximum limit of ${STUDY_SET_ITEM_LIMIT} items. Remove some items to add more.`);
                return;
            }
            ...
            if (cannotAccept > 0) {
                setError(`Added ${toAccept.length} items. ${cannotAccept} could not be added (limit of ${STUDY_SET_ITEM_LIMIT} reached).`);
            }
        } catch {
