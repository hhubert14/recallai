...
function createQuestionSuggestion(
    ...
    return {
        ...
        options: [
            { optionText: "A programming language", isCorrect: true, explanation: "Correct!" },
...
describe("useAISuggestionReview", () => {
    ...
    describe("accept", () => {
        it("calls flashcards API and removes suggestion on success", async () => {
            ...
            expect(global.fetch).toHaveBeenCalledWith(
                ...
                expect.objectContaining({
                    method: "POST",
                    body: JSON.stringify({ front: flashcard.front, back: flashcard.back }),
                })
            );
        ...
        it("sets error and keeps suggestion on API failure", async () => {
            ...
            global.fetch = vi.fn().mockReturnValue(
                mockFetchJsonResponse(mockResponse, false)
            );
            ...
            expect(result.current.error).toBe("Failed to add flashcard. Please try again.");
    ...
    describe("reject", () => {
        it("removes suggestion from list", () => {
            ...
            const fc2 = createFlashcardSuggestion({ tempId: "fc-2", front: "Another card" });
    ...
    describe("cancelEdit", () => {
        it("clears edit state without modifying suggestions", () => {
            ...
            expect((result.current.suggestions[0] as FlashcardSuggestion).front).toBe("Original front");
        });
    });
