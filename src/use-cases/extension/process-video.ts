...
export const processVideo = async (
    ...
    if (processType === "automatic") {
        ...
        console.log(`Processing video ${videoId} from ${videoUrl} automatically...`);
        ...
        let response = await fetch(`${request.nextUrl.origin}/api/v1/videos/${encodedVideoUrl}/educational?${queryParams.toString()}`, {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
            throw new Error(`Error processing video: ${videoData.error || "Unknown error"}`);
        }
        ...
        response = await fetch(`${request.nextUrl.origin}/api/v1/videos/${encodedVideoUrl}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Cookie": request.headers.get("cookie") || ""
        ...
        const createdVideo = responseJson.videoData || responseJson.data || responseJson;

        if (!response.ok) {
            throw new Error(`Error creating video: ${JSON.stringify(responseJson.error || "Unknown error")}`);
        }
        ...
        if (!createdVideo || !createdVideo.video_id) {
            throw new Error("Invalid response: Missing video_id in created video data");
        }

        const video_id_num = createdVideo.id;

        const title = videoData.videoData.snippet.title || "No Title";
        const description = videoData.videoData.snippet.description || "No Description";
        const transcript = videoData.transcript || "No Transcript";
        console.log("Video processed successfully:", { title, description, transcript });

        response = await fetch(`${request.nextUrl.origin}/api/v1/videos/${encodedVideoUrl}/summarize`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Cookie": request.headers.get("cookie") || ""
        ...
        if (!response.ok) {
            throw new Error(`Error generating summary: ${summaryData.error || "Unknown error"}`);
        }

        const summary = summaryData.content || "No Summary";
        console.log("Summary generated successfully:", summary);

        response = await fetch(`${request.nextUrl.origin}/api/v1/videos/${encodedVideoUrl}/questions`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Cookie": request.headers.get("cookie") || ""
        ...
        if (!response.ok) {
            throw new Error(`Error generating questions: ${questionsData.error || "Unknown error"}`);
        }
