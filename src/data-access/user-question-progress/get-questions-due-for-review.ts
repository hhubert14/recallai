...
export async function getQuestionsDueForReview(userId: string): Promise<QuestionForReviewDto[]> {
    if (!userId) {
        return [];
    }
    ...
    try {
        ...
        const questionsForReview: QuestionForReviewDto[] = data.map((progress: any) => ({
            id: progress.id,
            ...
            options: progress.questions.question_options.map((option: any) => ({
                id: option.id,
        ...
        logger.db.error("Error fetching questions due for review", error, { userId });
        return [];
    }
}
