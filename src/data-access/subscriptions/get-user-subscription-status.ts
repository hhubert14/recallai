...
export async function getUserSubscriptionStatus(userId: string): Promise<UserSubscriptionStatus> {
    ...
    try {
        ...
        if (!userData?.is_subscribed) {
            logger.subscription.debug('User not subscribed according to users table', { userId, is_subscribed: userData?.is_subscribed });
            return { isSubscribed: false };
        ...
        if (subscriptionError || !subscriptionData) {
            ...
            logger.subscription.debug('User marked as subscribed but no active subscription found', { userId, subscriptionError });
 ...
 */
export async function getUserSubscriptionStatusWithServiceRole(userId: string): Promise<UserSubscriptionStatus> {
    ...
    try {
        ...
        if (userError) {
            logger.subscription.error('Error fetching user subscription status with service role', userError, { userId });
            return { isSubscribed: false };
        }

        // If not subscribed according to users table, return early
        if (!userData?.is_subscribed) {
            logger.subscription.debug('User not subscribed according to users table (service role)', { userId, is_subscribed: userData?.is_subscribed });
            return { isSubscribed: false };
        }        // If subscribed, get detailed subscription info from subscriptions table
        const { data: subscriptionData, error: subscriptionError } = await supabase
            .from('subscriptions')
        ...
        if (subscriptionError || !subscriptionData) {
            // User might be marked as subscribed but no active subscription found
            // Return basic subscription status
            logger.subscription.debug('User marked as subscribed but no active subscription found (service role)', { userId, subscriptionError });
            return { isSubscribed: userData.is_subscribed };
        }        return {
            isSubscribed: true,
            status: subscriptionData.status,
        ...
        logger.subscription.error('Error fetching subscription status with service role', error, { userId });
        return { isSubscribed: false };
    }
}
