    ...
    try {
        ...
        if (error) {
            logger.subscription.error(
                "Error updating user Stripe customer ID",
                error,
        ...
        logger.subscription.error(
            "Exception updating user Stripe customer ID",
            error,
            { userId, stripeCustomerId }
        );
        return false;
...
export async function getUserStripeCustomerId(
    ...
    try {
        ...
        if (error) {
            logger.subscription.error(
                "Error getting user Stripe customer ID",
                error,
                { userId }
            );
            return null;
        ...
        logger.db.error("Exception getting user Stripe customer ID", error, {
            userId,
        });
        return null;
    }
